    ...
    <div className={styles.tutorialContainer}>
      <div className={`${styles.card} ${styles[`card-${currentIndex + 1}`]}`}>
        ...
        {cards[currentIndex].image && (
          <Image
            ...
            width={cards[currentIndex].width}
            height={cards[currentIndex].height}
          />
        )}
        <div>
          {cards[currentIndex].buttonLink && (
            <Button
              href={cards[currentIndex].buttonLink}
              text={cards[currentIndex].buttonText}
            />
          )}
        </div>
      </div>
      ...
      <div
        className={`${styles.talkAnimation} ${
          styles[`card-${currentIndex + 1}`]
        }`}
        ref={talkAnimationContainer}
      />
    </div>
  );
}
